...
def make_replacement_index(workbook:Workbook,report_name:str)->dict:
    ...
    for row in rows:
        ...
        temp_list += [('+管道名称',f"{sheet[log_dict['管道名称']+row].value}（{sheet[log_dict['探坑位置']+row].value}）")]
                ...
                if len(son_rows1) >0:
                    for son_row in son_rows1[:5]:
                        ...
                        temp_list +=[
                            ...
                            (f'&发现问题及位置描述{temp_count}',f'{sheet[log_dict['地表参照及位置描述']+son_row].value}（{sheet[log_dict['坐标X']+son_row].value}，{sheet[log_dict['坐标Y']+son_row].value}）'.replace('/','') ),
                            (f'&备注{temp_count}','/')
                        ]
                    if temp_count < 5:
                        temp_count += 1
