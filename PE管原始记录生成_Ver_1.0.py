...
def make_replacement_index(workbook:Workbook,report_name:str)->dict:
    ...
    for row in rows:
        ...
        temp_text_list:list[str]=['无设计资料'] if dc in '/' else ['有设计资料']
        temp_text_list+=['无安装资料'] if ac in '/' else ['有安装资料']
        temp_text_list +=['无竣工验收资料'] if str(fdate) in '/' else ['有竣工验收资料']
        temp_list+=[('+技术档案资料审查','，'.join(temp_text_list))]
...
if __name__ == '__main__':
    ...
    set_list:list[tuple[int,str,str]]=[
        (2,'模板文件','docx',r'E:\BaiduSyncdisk\成渝特检\模板文件与生成程序\记录、报告生成\PE管\金堂\PE管原始记录模板——金堂.docx'),
        (0,'数据源所在','',r'E:\BaiduSyncdisk\成渝特检\模板文件与生成程序\记录、报告生成\PE管\金堂'),
        (0,'签名图片所在','',r'E:\BaiduSyncdisk\成渝特检\模板文件与生成程序\记录、报告生成\PE管\电子签名'),
        (0,'输出文件所在','',r'E:\BaiduSyncdisk\成渝特检\模板文件与生成程序\记录、报告生成\PE管\记录文件\金堂'),
        (3,'是否检验签字',False,True), 
        (3,'是否审核签字',False,True), 
        (4,'审核人','','付飞'), 
        (4,'审核日期','','2025年11月12日'), 
    ...
    for report_name in sorted(list(all_names))[-1:]:
        try:
